...
class Conway extends Component {
	constructor(props) {
		...
		this.timeInterval = 1000 / this.props.fps;
		this.previousFrame = window.performance.now();
	}
	...
	_nextBoardState = timestamp => {
		this.setState(
			{
				globalAnimationID: window.requestAnimationFrame(
					this._nextBoardState,
				),
			},
			() => {
				let timeElapsed = timestamp - this.previousFrame;
				if (timeElapsed > this.timeInterval) {
					const grid = this._gotoNextState();
					this.previousFrame =
						timestamp - (timeElapsed % this.timeInterval);
					this.setState({ grid });
				}
